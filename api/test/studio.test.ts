...
describe('the script draft', () => {
  ...
  it('draws the start ceremony on the start line, before the gun, and edits it as a cue', async () => {
    ...
    const { estimates } = (await saved.json()) as {
      estimates: { firings: { eventId: string; meters: number; label: string; lat: number }[]; lines: { id: string; summary: string }[] };
    };
    ...
    expect(estimates.lines.find((l) => l.id === 'ceremony.countdown')?.summary).toBe('compte à rebours · ordre 1');
    ...
    expect(card).toMatch(/name="trigger.order"[^>]*value="1"/);
  });
